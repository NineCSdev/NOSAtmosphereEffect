...
class NeonReverseService : GLWallpaperService() {
    ...
    private val activeEngines = mutableSetOf<NeonReverseEngine>()

    override fun onCreateEngine(): Engine {
        val engine = NeonReverseEngine()
        activeEngines.add(engine)
        return engine
    }

    override fun getRenderer(): GLSurfaceView.Renderer {
        return NeonRenderer(applicationContext, isReverse = true)
    }
    ...
    inner class NeonReverseEngine : GLEngine() {
...
//                        handler.removeCallbacks(rotationRunnable)
        ...
        private fun updateRendererConfig() {
            ...
            animDuration = prefs.getLong("anim_duration", 1500L)
        }
    }
}